...
export type CashSectionMode = 'all' | 'expenses' | 'revenues'

const SECTION_TITLES: Record<CashSectionMode, string> = {
  all: 'Касса — все операции',
  expenses: 'Касса — расходы',
  revenues: 'Касса — доходы',
}

export function CashSectionPage({ mode }: { mode: CashSectionMode }) {
  ...
  return (
    <Card>
      ...
      >
        {selectedExpense ? (
          <Space style={{ marginBottom: 12 }}>
            <Button icon={<FileSearchOutlined />} onClick={() => navigate(`/cash/expenses/${selectedExpense.id}`)}>
              Открыть страницу
            </Button>
      ...
      <RequestDetailModal
        ...
        returnTo={
          requestDetail?.id
            ? ({
                pathname: selectedExpense ? `/cash/expenses/${selectedExpense.id}` : '/cash',
