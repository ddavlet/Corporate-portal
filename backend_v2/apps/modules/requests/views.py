...
class RequestApprovalsMixin:
    ...
    def auto_draft_submit_amount(self, request):
        ...
        # authorized to act on this exact row above, so the list-level role-based
        # payment-type visibility filter (accountant/cashier) must not hide it from this response.
...
class PortalRequestViewSet(
    ...
    def get_queryset(self):
        ...
        qs = Request.objects.filter(tenant=tenant).select_related(
            "vendor_ref", "contract_ref", "requester"
        )
