"""Run the amount/payed_at corporate card expense reconciliation pass on demand.

The n8n card expense upsert endpoints already trigger this after each import
...
Examples:
    python manage.py reconcile_card_expenses_by_amount --tenant=1
    python manage.py reconcile_card_expenses_by_amount  # all tenants
"""
...
class Command(BaseCommand):
    help = "Link unmatched Платежная карта requests to card expenses by amount + payed_at window."
    ...
    def handle(self, *args, **options):
        ...
        for tenant in tenants:
            linked = reconcile_card_expenses_by_amount_date(tenant=tenant)
            if linked:
