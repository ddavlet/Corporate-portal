...
В детальном виде начисления, для каждой строки, если связанная заявка в статусе `PAYED`: показывается "Выплачено X из Y" и кнопка "Выплатить" → модалка (сумма, по умолчанию — остаток) → вызывает новый эндпоинт. Там же — история выплат по строке (дата, сумма, кто провёл).
