...
- Отдельная секция "Сотрудники" — список + та же модалка добавления; живёт на странице настроек `/settings/payroll-config` (см. Часть 3), а не в самом `PayrollPage.tsx`.
...
### Статус заявки vs. факт реальной выплаты

Статус `Request` (заявки) **не трогаем**. `PAYED` по-прежнему означает то же, что и для всех остальных типов заявок: согласование пройдено (движок согласований — `approval_workflow.py` — общий для всех payment_type, и `APPROVED`/`PAYED` там уже несут узкий технический смысл, завязанный на цикл рассылки шагов; трогать эту логику ради ЗП — риск для протестированного общего кода, не делаем).

Факт "реально всё выплачено" — это отдельная, чисто аддитивная характеристика на уровне начисления, а не заявки: вычисляется агрегацией `paid_amount`/`remaining_amount` по всем строкам документа (`remaining_amount == 0` у каждой строки ⇒ документ полностью выплачен). Никакого нового поля/статуса в БД для этого не требуется — это `SerializerMethodField`/аннотация, не хранимое состояние.
...
- `PayrollLineSerializer` (используется в `PayrollDocumentDetailSerializer.lines`) дополняется read-only полями: `paid_amount`, `remaining_amount`, `payouts: [{id, amount, created_at, created_by}]`.
- `PayrollDocumentDetailSerializer` дополняется агрегатом `paid_amount` (сумма по всем строкам) — для шапки документа — и `request_status` (статус связанной заявки, чтобы фронт знал, показывать ли кнопку "Выплатить").
- `PayrollDocumentListSerializer` (список документов) дополняется тем же document-level `paid_amount` — для прогресса в списке, без захода в карточку.

### Фронтенд

**Страница документа (`PayrollDocumentDetailPage.tsx`, `/payroll/:id`) — основное место работы с выплатами.**
- В блок `Descriptions` (рядом с "Итого" / "Строк" / "Создан") добавляется пункт "Выплачено" — агрегат по документу.
- Таблица строк (`lineColumns`) получает три новые колонки: "Выплачено", "Остаток", и колонку с кнопкой "Выплатить" — активна только если `request_status === 'PAYED'`.
- Над таблицей — `rowSelection` (чекбоксы) + кнопка "Выплатить выбранным" над таблицей, вызывает bulk-эндпоинт для отмеченных строк.
- По каждой строке — разворачиваемая история выплат (`expandable` таблицы antd): дата, сумма, кто провёл.

**Список документов (`PayrollPage.tsx`, рядом с существующими тегами "Есть заявка" / "Оплачено", строки 421–425).** Тег "Оплачено" остаётся как есть (означает согласование заявки). Рядом добавляется отдельный индикатор реальной выплаты по документу, например "Выплачено 6/10" или прогресс-бар на основе `paid_amount`/`total_sum` — виден в списке без захода в карточку.
